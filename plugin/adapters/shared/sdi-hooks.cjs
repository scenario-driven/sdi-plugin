...
const CODEX_AGENTS_DISABLE_ENV = 'SDI_CODEX_AGENTS_DISABLE';
const CODEX_AGENT_SENTINEL = '# Generated by SDI Codex adapter.';
...
function codexHome() {
  return process.env[CODEX_HOME_ENV] || path.join(os.homedir(), '.codex');
}
function portFile() {
...
//
// Claude Code discovers plugin/agents/*.md directly. Codex custom agents are
// standalone TOML files under CODEX_HOME/agents or project .codex/agents. The
// plugin manifest does not currently package those agent files, so the Codex
// host needs a small registration bridge. It is strictly additive: Claude agent
...
function parseSimpleFrontmatter(raw) {
  ...
  for (const line of String(raw || '').split(/\r?\n/)) {
    ...
    if (!m) continue;
    out[m[1]] = parseScalarYamlValue(m[2]);
  }
  return out;
}
...
function parseClaudeAgentMarkdown(raw, sourcePath) {
  const m = String(raw || '').match(/^---\r?\n([\s\S]*?)\r?\n---\r?\n?/);
  if (!m) return null;
  const fm = parseSimpleFrontmatter(m[1]);
  const name = fm.name;
  if (!validAgentName(name)) return null;
  return {
    name,
    description: fm.description || `${name} SDI specialist.`,
    tools: fm.tools || '',
    model: fm.model || '',
    body: String(raw).slice(m[0].length).trim(),
    sourcePath,
  };
}

function readSdiClaudeAgents(rootArg) {
  const dir = path.join(rootArg || pluginRoot(), 'agents');
  try {
    return fs
      .readdirSync(dir)
      ...
      .map((entry) => {
        ...
        try {
          return parseClaudeAgentMarkdown(fs.readFileSync(file, 'utf8'), file);
        } catch {
          return null;
        }
      })
...
function isCodexModelName(value) {
  return typeof value === 'string' && /^gpt-[A-Za-z0-9_.:-]+$/.test(value);
}

function codexAgentInstructions(agent) {
  const sections = [];
  if (agent.body) sections.push(agent.body);
  if (agent.tools) {
    sections.push(
      [
        '## Source Tool Intent',
        `Claude tool intent: ${agent.tools}.`,
        'In Codex, use the tools available to this session and keep SDI hook gates binding.',
      ].join('\n'),
    );
  }
  sections.push(
    [
      '## Codex Adapter',
      'This agent is generated from the SDI Claude Code agent spec.',
      'Preserve the same specialist role, handoff discipline, and SDI scenario-first workflow.',
      'When a source Claude model is not a Codex model, inherit the parent Codex model and reasoning settings.',
    ].join('\n'),
  );
...
function buildCodexAgentToml(agent) {
  const lines = [
    `${CODEX_AGENT_SENTINEL} Source: ${agent.sourcePath}`,
    '# Remove this sentinel before editing manually; SDI refreshes generated files.',
    `name = ${tomlString(agent.name)}`,
    `description = ${tomlString(agent.description)}`,
    `developer_instructions = ${tomlString(codexAgentInstructions(agent))}`,
  ];
  if (isCodexModelName(agent.model)) {
    lines.push(`model = ${tomlString(agent.model)}`);
  } else if (agent.model) {
    lines.push(`# Source Claude model ${tomlString(agent.model)} is inherited in Codex.`);
  }
  return lines.join('\n') + '\n';
}
...
function installCodexAgents(rootArg, opts = {}) {
  const root = rootArg || pluginRoot();
  const targetDir = opts.targetDir || codexAgentsDir();
  const agents = readSdiClaudeAgents(root);
  const result = {
    targetDir,
    total: agents.length,
    written: 0,
  ...
  for (const agent of agents) {
    const target = path.join(targetDir, `${agent.name}.toml`);
    const next = buildCodexAgentToml(agent);
    let existing = null;
    try {
      existing = fs.readFileSync(target, 'utf8');
    } catch {}
    if (existing != null && !existing.startsWith(CODEX_AGENT_SENTINEL)) {
      result.skipped.push({ name: agent.name, path: target, reason: 'user-file-conflict' });
      continue;
    }
    if (existing === next) {
      result.unchanged += 1;
  ...
  _agentDirCache.delete(targetDir);
  return result;
}

function ensureCodexAgentsForHost(rootArg) {
  if (!isCodexPluginHost(rootArg)) return null;
  try {
    const result = installCodexAgents(rootArg);
    if (result.written > 0 || result.skipped.length > 0) {
      appendHookLog('codex_agents_install', {
    ...
    if (result.skipped.length > 0) {
      process.stderr.write(
        `[sdi] Codex agent install skipped ${result.skipped.length} existing user file(s) in ${result.targetDir}.\n`,
      );
    }
    return result;
  } catch (err) {
    const message = err && err.message ? err.message : String(err);
    appendHookLog('codex_agents_install_failed', { error: message });
    process.stderr.write(`[sdi] Codex agent install failed: ${message}\n`);
    return null;
  }
}
...
// Agent registry — read from host-native roots so a user-defined specialist is
// recognised without copying it into the plugin tree (#4/#11):
//   1. <cwd>/.claude/agents      — Claude project-local
//   2. <cwd>/.codex/agents       — Codex project-local
//   3. ~/.claude/agents          — Claude user-level
//   4. CODEX_HOME/agents         — Codex user-level
//   5. <pluginRoot>/agents       — SDI built-in Claude specs
// These mirror the host discovery roots the hook can observe, so a `name` the
// host can spawn is a `name` this registry recognises.
//
...
function parseTomlBasicString(rawValue) {
...
function loadAgentNameFromToml(file) {
  try {
    const raw = fs.readFileSync(file, 'utf8');
    const m = raw.match(/^\s*name\s*=\s*(.+?)\s*$/m);
    if (!m) return null;
    const name = parseTomlBasicString(m[1]);
...
function loadAgentNamesFromDir(dir) {
  ...
  try {
    for (const entry of fs.readdirSync(dir)) {
      const file = path.join(dir, entry);
      const name = entry.endsWith('.md')
        ? loadAgentNameFromMarkdown(file)
        : entry.endsWith('.toml')
          ? loadAgentNameFromToml(file)
          : null;
...
// Is `bareType` registered in any host-visible discovery root?
...
// Union of all registered agent names across host-visible roots (for hints).
    ...
    if (!isRegisteredAgent(bareType, cwd)) {
      ...
      process.stderr.write(
        `[sdi] note: agent_type "${agentType}" is unregistered — acting at L3 ` +
          `(autonomy capped; register in .claude/agents, .codex/agents, or the user agent dir to ` +
