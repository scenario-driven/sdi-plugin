...
- **Project model fields.** `Project { id, key, name, slug, cwds, description?, enabled, wiki_paths, created_at, updated_at }`. `enabled=false` makes the hook layer skip every mutating gate for the project's anchored cwds (`project-disabled` audit). `wiki_paths` defaults to `["docs"]` and drives the dashboard `WikiView`. `PUT /projects/:id` is PATCH-style; `DELETE /projects/:id` cascades the PROJ-scoped subtree and refuses by default when an `in_progress` task exists (`?force=true` overrides).
