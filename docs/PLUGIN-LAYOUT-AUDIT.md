...
| **No `components.json`** | Clawket carries `components.json` because its cli / daemon / web / desktop live in 7 separate repos and need explicit version pins. SDI is a single Cargo workspace, so `Cargo.toml [workspace.package].version` already pins cli + daemon + plugin shell (dashboard SPA at `plugin/web/` included) in lock-step; the separate `sdi-desktop` repository rides the daemon HTTP contract and ships on its own cadence, with no shared release manifest. A second pinning surface would only invite drift. |
